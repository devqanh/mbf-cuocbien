...
- `DatabaseSeeder` = gán quyền: **super_admin LUÔN** `syncPermissions(Permission::all())` (an toàn, tự nhận quyền mới). admin/ke_toan/chung_tu chỉ sync **1 lần** rồi set cờ `TruckingSetting sys.roles_initialized=1`; deploy sau bỏ qua → giữ chỉnh tay. ⚠️ Hệ quả: thêm permission mới về sau, admin/ke_toan/chung_tu KHÔNG tự nhận — phải bật tay ở /roles (chỉ super_admin auto).
