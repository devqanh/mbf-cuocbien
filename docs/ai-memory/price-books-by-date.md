...
**(2026-09-11: giá đã chuyển sang `prices` JSON theo LOẠI CONT + mẫu phẳng "Tải mẫu" — xem [[price-by-cont-type]]; parser báo giá gốc dưới đây vẫn chạy, quy về cột chung 20FT/40FT.)**

**Nhập từ FILE BÁO GIÁ GỐC** (dev/MBF-*.xlsx — layout báo giá nhiều mục, header 2 dòng): `parseQuotationRows($path)` đọc sheet **'import'** → mục 2 DRAYAGE (Loại=Connect/Disconnect theo tiêu đề 2.x.1/2.x.2, Điểm Hạ=cảng 2.x [HAI PHONG/LACH HUYEN/ICD QV/ICD TP], KIND=cột C, FROM=D, TO1..4=E..H, Distance=I, Transport40/20=J/K, Fuel40/20=L/M) + mục 3 BARGING (3.1 DRY/3.2 NOR → Loại=Non, KIND=DRY/NOR CONTAINER, FROM=D, Điểm Hạ=điểm đến HPP/LHP). **Bẫy đã xử lý trong parse:** cột KIND là Ô GỘP (chỉ dòng đầu nhóm có chữ) → KÉO XUỐNG (forward-fill, reset mỗi cảng/conn); KIND có xuống dòng trong ô → gộp `\s+`→1 space (cho cả FROM/TO) để khớp chuỗi định giá; khối ghi chú (TRANSPORTATION FEE FOR COMBINATION/COMEBACK/DETENTION) → dừng bắt tuyến (tránh dòng rác KIND "-"). `importQuotationToBook($bookId,$path,$replace,$sheet)` → savePriceBookRows/importPriceRows. parseQuotationRows nhận tham số `$sheet` (mặc định 'import'). **Luồng UI 2 bước (có xác nhận):** nút "Nhập báo giá gốc" → popup: chọn file (đọc tên sheet client-side qua XLSX) → **SELECT sheet** → **"Kiểm tra"** (POST `/price-quote-validate` → `validateQuotation($path,$sheet)` dry-run: tổng dòng + theo Loại Con/Discon/Non + nhóm KIND + cảnh báo, KHÔNG ghi DB) → chỉ khi OK mới bật **"Import báo giá"** (POST `/price-quote-import` file+book+sheet+replace, xác nhận ghi đè). Test: MBF-202606-02 → 576 dòng (Discon 266/Con 306/Non 4), lô sà lan ICDTP→HPP đúng 3.315.432+492.217. File thiếu sheet 'import' → báo lỗi.

Verify (transaction-rollback): 2 book A(1-15/6)/B(16-30/6) → lô 5/6→A, 20/6→B, 20/8→chưa khớp; backfill 1 book/khách; CRUD + cascade OK. Liên quan [[lo-hang-location-filters]], [[cost-item-auto-vat]], [[mbf-import]].
