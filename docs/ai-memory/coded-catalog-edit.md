...
**Lô lưu TÊN địa điểm thay vì KÝ HIỆU — đã dọn (commit `d9dafdf`, 2026-06-25):** import Excel lưu thẳng tên cột (vd "TÂN VŨ","HATECO","ICD Quế Võ") + `registerLocationCode` tạo bản `code==name` khi chưa có alias → lô `from_loc/to_loc/barge_drop` chứa TÊN (có dấu TV) không khớp ký hiệu bảng giá. **KÝ HIỆU không bao giờ có dấu TV; chỉ TÊN mới có** (dấu hiệu nhận lỗi). Migration `2026_06_25_000009` (data-driven, idempotent): (1) migrate lô TÊN→ký hiệu sạch theo map name→code (location code không dấu), "ICD Quế Võ"→"ICDQV"; (2) chỉ sau khi lô hết tham chiếu mới XÓA địa điểm lỗi (code có dấu / code==name trùng alias) — giữ alias name→code hợp lệ. **Chặn tái phát:** `saveShipment` canon from/to/bargeDrop qua `$this->locCode()` (HandlesTripAndDrivers, public; map tên→ký hiệu) — UI vốn gửi ký hiệu nên không đổi, chỉ import gửi tên bị canon. Xem [[lo-hang-location-filters]].
