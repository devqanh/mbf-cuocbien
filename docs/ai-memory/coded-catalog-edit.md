...
- **Kho: ký hiệu ĐÃ LƯU vẫn sửa được nhưng CHẶN TRÙNG nhóm khác** (user 2026-09-11: "đây như id của kho" — lô lưu kho theo KÝ HIỆU). Cờ `codeEditable` (groups.js, CHỈ Kho); Địa điểm vẫn khóa `codeSaved` (commit e6e7add). FE (config.jsx): gõ trùng/để trống ở header nhóm đã lưu → giữ NHÁP `codeDrafts` (KHÔNG ghi codeArr, vì ghi vào là 2 nhóm gộp ngay khi đang gõ) + `hasCodeDraft` chặn lưu; so khớp `codeIdent` = bỏ dấu + khoảng trắng + không phân biệt hoa/thường (như normalizedCodeMap). BE: `assertWarehouseCodesDistinct` trong reconcileLookup (bỏ qua addOnly) → 422 = 2 nhóm kho ĐÃ CÓ cùng ký hiệu. Tên mới / kho cũ chưa có ký hiệu vẫn nhận ký hiệu đang có (= thêm tên vào nhóm). ⚠ Đổi ký hiệu KHÔNG cascade text: `trucking_shipments.kho`, `price_rows.to1..to4`, `route_fees.route` giữ ký hiệu cũ — vẫn khớp chỉ khi nhóm còn 1 TÊN = ký hiệu cũ (khoResolve/normalizedCodeMap map cả tên).
