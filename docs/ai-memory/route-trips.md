...
- **Chuyến CHƯA HOÀN THÀNH** (khách phản hồi 2026-09-11: gán xe xong là muốn thấy lộ trình ngay, không đợi xe ra): `routeTripByDate($date, $withPlanned = true)` — CHỈ `LoTrinhController::data` truyền true. Lô có `bks_vao` nhưng CHƯA ra (self: `gio_xe_ra` trống · none: `gio_xe_ra_xe` trống · other: cont ra hộ chưa có `gio_xe_ra`) → `truck.planned[]` (leg + `planned:true`), xếp ngày theo **`gio_xe_den ?: gio_den_du_kien`** trong khung 08:00→08:00; bỏ lô đang là cont ra hộ của lô khác. Có giờ ra → thành leg hoàn thành (ngày theo GIỜ RA, có thể sang ngày sau). TÁCH khỏi `legs`/`payGroups` nên chi lái / kỳ lương / báo cáo / chốt ngày KHÔNG đổi (đã so JSON mặc định trước/sau = giống hệt). FE: TripNode chấm rỗng nét đứt + chip "Chưa hoàn thành", giờ "Dự kiến HH:MM"/"Vào HH:MM", điểm hạ + mũi tên vào nó làm mờ (`PointChain dimDrop`, user yêu cầu); xe chỉ có chuyến chưa xong ẩn nút Chi lái; Chốt ngày chỉ xét xe có legs (`payTrucks`).
- **Link sang Lô hàng** (`hrefOf`): `?q=<cont, chưa có cont thì booking>&open=<hashid>` → ShipmentsApp mở ĐÚNG lô (khớp hashid/id → số cont → dòng đầu; số cont bị dùng lại ở nhiều lô). ShipmentsApp mở bằng link (`fromLink` = có `?q`) BỎ QUA bộ lọc đã lưu localStorage `trk:lohang:filters` — lỗi 2026-09-11: tab "Đã ra"/ngày đóng hàng/thanh lý đã lưu lọc mất chuyến chưa ra → danh sách rỗng, popup không mở — và KHÔNG ghi đè bộ lọc đã lưu lúc mở trang (`skipPersist`); không khớp lô nào → toast báo.
