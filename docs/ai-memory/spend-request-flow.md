...
- **Dự kiến vs thực tế (user 2026-06-22):** số lái xe gửi = **dự kiến** → cột `est_amount` (migration `2026_06_22_000001`, decimal 14,2 nullable). createSpendRequest/updateSpendRequestByOwner set `est_amount = amount = số gửi` (amount mirror dự kiến lúc chờ duyệt). Kế toán mở CostModal (quan-ly-xe→Chi phí): ô **"Số tiền thực tế (chi)"** = `amount` (sửa được), kèm hint "Dự kiến lái xe gửi: X" + nút "dùng số này" (chỉ hiện khi `estAmount != null`). **Báo cáo vẫn dùng `amount`** (thực tế) — không đổi. saveVehicleManagement PRESERVE `est_amount` theo id (kế toán không sửa) — như created_by/cancelled. costsOut + spendRequestHistory trả `estAmount`; lịch sử lái xe hiện dự kiến + "Thực chi: Y" nếu kế toán chốt khác. Form spend đổi nhãn "Số tiền dự kiến". Phiếu kế toán tạo trực tiếp → est_amount NULL (label vẫn "Số tiền").
