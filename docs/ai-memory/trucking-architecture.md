...
**PHÍ XE (trip cost) ĐÃ Ở BACKEND sẵn** (khác bảng kê): `computeTripCosts` query lô theo khoảng `gio_xe_ra` (KHÔNG load toàn bộ) + `tripSuggest` (PHP) định giá; "Tính lại" = `tripBatchContext` chỉ re-price lô trong kỳ. Client `trip-cost.jsx` chỉ hiển thị/sửa → ổn cho 10–20k lô.

**Index trucking_shipments**: đã có sẵn `customer_id`(FK), `gio_xe_ra`, `sheet`, `vehicle_id`, `from/to_location_id`; THÊM `cont_ra` + `gio_den_du_kien` (mig `_000008`) + **composite `(customer_id, cont_ra)`** `tsh_customer_contra_idx` (mig `_000009`, statementCandidates orderBy cont_ra). Bảng con đã đủ index sẵn: cost_lines `(shipment_id, amount)`, attachments `(owner_type, owner_id, group)`, vehicle_costs `vehicle_id/created_by/cost_type_id`, statement_lines/trip_cost_lines theo FK — KHÔNG cần thêm.
